...
describe('Game2048Detail', () => {
	...
	it('renders the title and starts a fresh game with score 0', () => {
		const { container } = render(Game2048Detail, { props: { data: { title: '2048' } } });
	...
	it('loads a previously saved best score on mount', () => {
	...
	it('persists a new best score once the current score surpasses it', async () => {
		render(Game2048Detail, { props: { data: { title: '2048' } } });

		await fireEvent.keyDown(window, { key: 'ArrowLeft' });

		expect(localStorage.getItem('game2048-best-score')).toBe('4');
	});
});
