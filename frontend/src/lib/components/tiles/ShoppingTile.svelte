<script lang="ts">
	...
	let summary = $state<ShoppingData | null>(null);

	async function refresh() {
		try {
			summary = await api.widgetSummary<ShoppingData>(widgetId);
		} catch {
	...
	// Checking an item off from the tile should check it off, not fall
	// through to TileCard's button and navigate to the detail page.
	async function check(event: Event, id: number) {
...
<TileCard {widgetId}>
	<div class="widget">
		...
		{#if summary}
			{@const openItems = summary.items.filter((item) => !item.checked)}
			{#if openItems.length > 0}
				<ul class="items">
					{#each openItems as item (item.id)}
						<li>
							<label>
								<input type="checkbox" onclick={(event) => check(event, item.id)} />
