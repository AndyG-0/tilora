...
def test_add_package_returns_stored_fields(tmp_db):
    package = db.add_package("packages", "alice", "1Z999AA1", "Birthday gift")

    assert package["widget_id"] == "packages"
    assert package["user_id"] == "alice"
...
def test_add_package_defaults_label_to_none(tmp_db):
    package = db.add_package("packages", "alice", "1Z999AA1")

    assert package["label"] is None


def test_list_packages_scoped_to_widget(tmp_db):
    db.add_package("packages", "alice", "mine")
    db.add_package("other-widget", "alice", "not-mine")

    numbers = [p["tracking_number"] for p in db.list_packages("packages", "alice")]
    assert numbers == ["mine"]


def test_list_packages_scoped_to_user(tmp_db):
    # Packages is per-user (see the packages table comment) — each household
    # member tracks their own deliveries, so one user's list must not
    # include another user's packages for the same widget.
    db.add_package("packages", "alice", "alices")
    db.add_package("packages", "bob", "bobs")

    numbers = [p["tracking_number"] for p in db.list_packages("packages", "alice")]
    assert numbers == ["alices"]


def test_list_all_packages_for_widget_returns_every_users_packages(tmp_db):
    db.add_package("packages", "alice", "alices")
    db.add_package("packages", "bob", "bobs")

    numbers = {p["tracking_number"] for p in db.list_all_packages_for_widget("packages")}
    assert numbers == {"alices", "bobs"}
...
def test_list_packages_orders_active_by_earliest_eta_first(tmp_db):
    later = db.add_package("packages", "alice", "later")
    sooner = db.add_package("packages", "alice", "sooner")
    db.add_package("packages", "alice", "no-eta")
...
def test_remove_package_deletes_and_returns_row(tmp_db):
    ...
    assert db.list_packages("packages", "alice") == []
