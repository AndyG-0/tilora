...
def test_is_configured_tcp_requires_host():
    assert docker_client.is_configured(TCP_SETTINGS)
    assert not docker_client.is_configured({"connection": "tcp", "host": ""})


def test_is_configured_socket_defaults_to_true():
    assert docker_client.is_configured(SOCKET_SETTINGS)
    assert docker_client.is_configured({})
...
async def test_fetch_containers_over_tcp_maps_fields():
    ...
    containers = await docker_client.fetch_containers(TCP_SETTINGS)
...
async def test_fetch_containers_over_socket_maps_fields():
    respx.get("http://docker/containers/json", params={"all": "true"}).mock(
        return_value=httpx.Response(200, json=CONTAINERS_RESPONSE)
    )

    containers = await docker_client.fetch_containers(SOCKET_SETTINGS)
...
def test_container_dict_falls_back_to_id_without_names():
    ...
    result = docker_client._container_dict(entry)
...
async def test_fetch_containers_raises_on_connect_error():
    ...
    with pytest.raises(docker_client.DockerError):
        await docker_client.fetch_containers(TCP_SETTINGS)
