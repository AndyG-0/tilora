...
async def ask(
    ...
    async with MCPToolSource(load_mcp_server_configs()) as mcp_source:
        ...
        tools = [tool for plugin in plugins for tool in plugin.get_ai_tools()] + await mcp_source.tools()
        provider = AIProvider(ToolBridge(tools))
        return await provider.run_prompt(text, system_prompt=system_prompt)
