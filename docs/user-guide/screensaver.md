# Screensaver & Ambient Display

When left unattended, Tilora features a customizable ambient screensaver designed for wall-mounted displays and dedicated kiosks. It cycles through your favorite dashboard widgets with stylized text animations, preventing screen burn-in while displaying ambient information.

---

## Configuring the Screensaver

Navigate to **Settings → Your Settings → Screensaver**:

| Setting | Description | Default |
|---|---|---|
| **Enable on this device** | Toggle ambient screensaver on the current screen. | Enabled |
| **Idle timeout (seconds)** | How long the screen must remain untouched before launching. | `60` seconds |
| **Rotation interval (seconds)** | Duration each widget displays before transitioning to the next. | `15` seconds |
| **Animation Style** | The visual rendering effect for text-heavy widgets (see below). | `Marquee` |
| **Reading pause (seconds)** | How long fully revealed text holds still before advancing. | `4` seconds |
| **Widget Selection** | Checkboxes to choose which enabled widgets participate in the rotation. | All eligible |

Tap **Test screensaver** to instantly preview your settings without waiting for the idle timer.

---

## Animation Styles

Tilora includes four animations for rendering textual information (such as AI briefings, news headlines, Discord messages, or sports scores):

### 1. Marquee (Continuous Scrolling)
Smooth horizontal scrolling text banner across the screen, reminiscent of classic news tickers and financial boards.

### 2. Matrix (Digital Rain)
Iconic falling green (or customizable) digital code glyphs that materialize into your news headlines, weather alerts, or messages.

### 3. Flipboard (Split-Flap)
Simulates mechanical split-flap departure boards found in train stations and airports. Letters flip rapidly until settling into words.
- Patterns: **Top to bottom** or **Random**.

### 4. LED Dot-Matrix
Emulates a bright retro LED display board.
- **Custom LED Color**: Choose amber, cyan, emerald green, ruby red, or any custom hex color with the color picker.

---

## Screensaver-Eligible Widgets

The screensaver dynamically renders custom full-screen views for:
- **Clocks & Date**: Large analog, digital, binary, or word clock faces.
- **Photos**: Fullscreen photo slideshow with smooth crossfades.
- **Weather**: High-contrast temperature, condition icon, and severe weather alert banners.
- **AI Daily Briefing**: Animated summary of the morning report.
- **Sports**: Live scores and upcoming game schedule cards.
- **Flights**: Live overhead radar and aircraft tracking banners.
- **Discord & RSS**: Animated headline streams.
- **NASA APOD**: Full-resolution Astronomy Picture of the Day.
